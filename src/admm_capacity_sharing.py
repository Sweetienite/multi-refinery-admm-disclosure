...
class ADMMParams:
    ...
    primal_tol: float = 1e-3   # Primal residual convergence threshold
    dual_tol: float = 1e-3     # Dual residual convergence threshold
    profit_scale: float = 1e6  # Profit normalisation factor
...
class ADMMResult:
    ...
    profit_a: float
    profit_b: float
    allocation_a: float     # Plant A consensus capacity allocation (kt)
    allocation_b: float     # Plant B consensus capacity allocation (kt)
    iterations: int
    final_primal_residual: float
    final_dual_residual: float
    history: ADMMHistory = field(default_factory=ADMMHistory)
...
# ---------------------------------------------------------------------------
# Sub-problem: single-refinery LP with capacity penalty
# ---------------------------------------------------------------------------

def _solve_subproblem(
    params: RefineryParams,
    shared_streams: list[str],
    z_k: float,
    lambda_k: float,
    rho: float,
    profit_scale: float,
    ...
    """
    Solve single-refinery sub-problem with ADMM augmented term.

    Returns (local_allocation, local_profit_CNY).
    """
    m = pyo.ConcreteModel()
    streams = (set(params.revenue) | set(params.cost)
               | set(params.capacity_ub)
               | {s for s, _, _ in params.conversions}
               | {t for _, t, _ in params.conversions})

    m.S = pyo.Set(initialize=sorted(streams))
    m.x = pyo.Var(m.S, within=pyo.NonNegativeReals)

    for r, ub in params.raw_ub.items():
        if r in streams:
            m.x[r].setub(ub)
    for p, ub in params.product_ub.items():
        if p in streams:
            m.x[p].setub(ub)
    for s, ub in params.capacity_ub.items():
        if s in streams:
            m.x[s].setub(ub)

    m.conv = pyo.ConstraintList()
    for s_in, s_out, ratio in params.conversions:
        m.conv.add(m.x[s_out] <= ratio * m.x[s_in])

    # Local capacity allocation variable (shared dimension)
    cap_streams_in_s = [s for s in shared_streams if s in streams]
    if cap_streams_in_s:
        local_alloc = sum(m.x[s] for s in cap_streams_in_s)
    else:
        local_alloc = 0.0

    # Augmented Lagrangian penalty: λ·y_i + (ρ/2)·||y_i - z_k||^2
    rev = sum(params.revenue.get(s, 0.0) * m.x[s] for s in streams) / profit_scale
    cost = sum(params.cost.get(s, 0.0) * m.x[s] for s in streams) / profit_scale
    penalty = lambda_k * local_alloc + (rho / 2.0) * (local_alloc - z_k) ** 2

    m.obj = pyo.Objective(expr=rev - cost - penalty, sense=pyo.maximize)
    ...
    profit = float(pyo.value(rev - cost)) * profit_scale
    if cap_streams_in_s:
        alloc = float(pyo.value(local_alloc))
    else:
        alloc = 0.0
    return alloc, profit
...
def run_admm(
    ...
    """
    Run ADMM capacity-sharing approximation for two refineries.

    The shared constraint is: y_A + y_B <= shared_cap_ub
    where y_A and y_B are each refinery's local allocation of the shared
    capacity resource.
    ...
    rho = admm_params.rho
    scale = admm_params.profit_scale
    shared_streams = exchange.shared_cap_streams
    cap_ub = exchange.shared_cap_ub

    # Initialise dual variables and consensus
    lambda_a = 0.0
    lambda_b = 0.0
    z = cap_ub / 2.0  # Initial consensus estimate

    history = ADMMHistory()
    y_a_prev, y_b_prev = 0.0, 0.0

    for k in range(admm_params.max_iter):
        # x-updates (parallel sub-problems)
        y_a, profit_a = _solve_subproblem(
            params_a, shared_streams, z, lambda_a, rho, scale
        )
        y_b, profit_b = _solve_subproblem(
            params_b, shared_streams, z, lambda_b, rho, scale
        )

        z_prev = z
        # z-update: project onto feasible set [0, cap_ub]
        z = min(cap_ub, max(0.0, (y_a + y_b) / 2.0 + (lambda_a + lambda_b) / (2.0 * rho)))
    ...
    return ADMMResult(
        ...
        allocation_b=y_b,
        iterations=admm_params.max_iter,
        final_primal_residual=history.primal_residuals[-1],
        final_dual_residual=history.dual_residuals[-1],
        history=history,
    )
